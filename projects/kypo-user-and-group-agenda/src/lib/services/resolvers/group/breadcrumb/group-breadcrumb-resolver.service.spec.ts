...
import { GROUP_EDIT_PATH, GROUP_NEW_PATH } from '../../../../model/client/default-paths';
...
describe('GroupBreadcrumbResolver', () => {
  ...
  it('should resolve to create breadcrumb when url ends with new group path', () => {
  ...
  it('should resolve to group name when has id param and group resolver returns group', (done) => {
  ...
  it('should resolve to edit group name when has id param and group resolver returns group', (done) => {
    const paramMapSpy: jasmine.SpyObj<ParamMap> = jasmine.createSpyObj('ParamMap', ['has']);
    paramMapSpy.has.and.returnValue(true);
    const routeSnapshot = new ActivatedRouteSnapshot();
  ...
  it('should resolve to empty when has id param and group resolver returns undefined', (done) => {
    const paramMapSpy: jasmine.SpyObj<ParamMap> = jasmine.createSpyObj('ParamMap', ['has']);
    paramMapSpy.has.and.returnValue(true);
    const routeSnapshot = new ActivatedRouteSnapshot();
    const routerState: RouterStateSnapshot = { root: undefined, url: `something` };
  ...
  it('should resolve to empty when has id param and group resolver throws error', (done) => {
    const paramMapSpy: jasmine.SpyObj<ParamMap> = jasmine.createSpyObj('ParamMap', ['has']);
    paramMapSpy.has.and.returnValue(true);
    const routeSnapshot = new ActivatedRouteSnapshot();
    const routerState: RouterStateSnapshot = { root: undefined, url: `something` };
  ...
  it('should resolve to empty observable when does not match new group path and has no id param', (done) => {
    const paramMapSpy: jasmine.SpyObj<ParamMap> = jasmine.createSpyObj('ParamMap', ['has']);
